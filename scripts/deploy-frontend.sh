...
echo "==> Attaching apex domains to the project"
...
cf_record() { # type name content proxied
  curl -sS -X POST "https://api.cloudflare.com/client/v4/zones/${ZONE_ID}/dns_records" \
    -H "Authorization: Bearer ${CF_API_TOKEN}" -H "Content-Type: application/json" \
    -d "{\"type\":\"$1\",\"name\":\"$2\",\"content\":\"$3\",\"proxied\":$4,\"ttl\":1}" \
    | python3 -c "import sys,json;r=json.load(sys.stdin);print('   ok' if r['success'] else '   ' + str(r['errors']))"
}
echo "   @   -> 76.76.21.21 (Vercel)";        cf_record A "@" "76.76.21.21" false
echo "   www -> cname.vercel-dns.com";        cf_record CNAME "www" "cname.vercel-dns.com" false
echo "   *   -> rentium.ca (Cloudflare)";      cf_record CNAME "*" "$DOMAIN" true

echo "==> Deploying Cloudflare vanity-subdomain Worker"
...
1. WILDCARD (*.rentium.ca showcase subdomains): Cloudflare terminates wildcard
   TLS and the Worker fetches the matching /l/<slug> page from Vercel. Do not
   attach *.rentium.ca directly to the Vercel project.
