...
export default function LeasesPage() {
  ...
  const tenantLine = (l: LeaseRow) => {
    const names =
      l.tenant_names && l.tenant_names.length > 0
        ? l.tenant_names
        : l.primary_tenant_name
          ? [l.primary_tenant_name]
          : [];
    if (names.length === 0)
      return <span className="text-ink-4">No tenants yet</span>;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-3">
            Every agreement, with who&apos;s on it at a glance.
          </p>
        </div>
        <Button
          className=""
          ...
          {TABS.map((t) => (
            ...
            >
              ...
              <span className="ml-1 opacity-70">
                {t.value === 'ALL'
                  ? leases.length
                  : leases.filter((l) => l.status === t.value).length}
              </span>
            </button>
          ))}
        </div>
            ...
            <table className="min-w-full divide-y divide-line text-sm">
              ...
              <tbody className="divide-y divide-line">
                {filtered.map((l) => (
                  <tr
                    key={l.id}
                    className="hover:bg-brand-soft/60 cursor-pointer"
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right font-medium hidden sm:table-cell">
                      {money(l.total_rent)}
                      <span className="text-ink-4 font-normal">/mo</span>
                    </td>
                    <td className="px-4 py-3">
                      <Badge
                        variant="outline"
                        className={STATUS_STYLES[l.status] || 'bg-canvas'}
                      >
                        {l.status_display}
                      </Badge>
                    </td>
