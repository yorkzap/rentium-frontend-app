...
const STATUS_STYLES: Record<string, string> = {
  ...
  DRAFT: "bg-canvas text-ink-3 border-line",
  EXPIRED: "bg-surface-sunken text-ink-3 border-line",
...
export default function LeasesPage() {
  ...
  const tenantLine = (l: LeaseRow) => {
    ...
    if (names.length === 0) return <span className="text-ink-4">No tenants yet</span>;
    const shown = names.slice(0, 2).join(", ");
    const extra = names.length - 2;
    return (
      <span className="text-ink-2">
        {shown}
        {extra > 0 && <span className="text-ink-4"> +{extra} more</span>}
      </span>
    );
  };
    ...
    <div className="container max-w-6xl py-6 px-4 sm:px-6 space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-ink flex items-center gap-2">
            <FileText className="h-6 w-6 text-brand" /> Leases
          </h1>
          <p className="text-sm text-ink-3">Every agreement, with who's on it at a glance.</p>
        </div>
        <Button className="" onClick={() => router.push("/dashboard/leases/create")}>
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
          ...
          {TABS.map((t) => (
            <button key={t.value} type="button" onClick={() => setTab(t.value)}
              className={`rounded-full px-3 py-1 text-sm transition-colors ${
                tab === t.value ? "bg-brand text-white" : "bg-surface-sunken text-ink-2 hover:bg-line"
              }`}>
              {t.label}
        ...
        <div className="relative w-full sm:w-64">
          <Search className="h-4 w-4 absolute left-2.5 top-2.5 text-ink-4" />
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-line text-sm">
              ...
              <tbody className="divide-y divide-line">
                {filtered.map((l) => (
                  <tr key={l.id} className="hover:bg-brand-soft/60 cursor-pointer"
                    onClick={() => router.push(`/dashboard/leases/${l.id}`)}>
                    <td className="px-4 py-3">
                      <p className="font-medium text-ink">{l.property_name || l.group_name}</p>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <Users className="h-3.5 w-3.5 text-ink-5 flex-shrink-0" />
                        {tenantLine(l)}
                      </div>
                      {/* mobile: term inline since the column is hidden */}
                      <p className="text-xs text-ink-4 md:hidden mt-0.5">
                        {fmtDate(l.start_date)} – {l.is_month_to_month ? "Month-to-month" : fmtDate(l.end_date)}
                      </p>
                    </td>
                    <td className="px-4 py-3 text-ink-3 hidden md:table-cell whitespace-nowrap">
                      {fmtDate(l.start_date)} – {l.is_month_to_month ? "Month-to-month" : fmtDate(l.end_date)}
                    </td>
                    <td className="px-4 py-3 text-right font-medium hidden sm:table-cell">
                      {money(l.total_rent)}<span className="text-ink-4 font-normal">/mo</span>
                    </td>
                    <td className="px-4 py-3">
                      <Badge variant="outline" className={STATUS_STYLES[l.status] || "bg-canvas"}>
                        {l.status_display}
                      </Badge>
                    </td>
                  </tr>
