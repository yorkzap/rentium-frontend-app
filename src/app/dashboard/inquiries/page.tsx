...
export default function InquiriesPage() {
  ...
  const act = async (id: string, path: string, body?: object) => {
    const res = await fetch(
      `${DJANGO_API_URL}/showcase/inquiries/${id}/${path}/`,
      {
        method: 'POST',
        headers: {
        ...
        <ul className="space-y-3">
          {items.map((i) => (
            <li
              ...
              className={cn(
                'card p-5',
                i.status === 'NEW' && 'border-[hsl(var(--brand))]'
              )}
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm text-[hsl(var(--ink-4))]">
                    about{' '}
                    <Link
                      href={`/dashboard/properties/${i.property}`}
              ...
              {i.move_in_target && (
                <p className="mt-2 text-xs text-[hsl(var(--ink-4))]">
                  Hoping to move in{' '}
                  {new Date(`${i.move_in_target}T00:00:00`).toLocaleDateString(
                    'en-CA',
                    {
                      month: 'long',
                      day: 'numeric',
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {/* Reply now opens the continuing thread in Messages — the
                    inquiry and every later message live in one place, and the
                    prospect gets each reply by email with a link back to their
                    own chat page. */}
                {i.conversation ? (
                  <Link
                    href={`/dashboard/messages?c=${i.conversation}`}
                    onClick={() =>
                      i.status === 'NEW' && act(i.id, 'mark_replied')
                    }
                  ...
                  <a
                    href={`mailto:${i.email}?subject=Re: ${i.property_name}`}
                    onClick={() =>
                      i.status === 'NEW' && act(i.id, 'mark_replied')
                    }
                    className="flex items-center gap-1.5 rounded-lg bg-[hsl(var(--brand))] px-3 py-1.5 text-sm font-medium text-white hover:bg-[hsl(var(--brand-hover))]"
                ...
                {i.appointment ? (
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={!when}
                      onClick={() => {
                        act(i.id, 'to_appointment', {
                          starts_at: new Date(when).toISOString(),
                        });
                        setScheduling(null);
                        setWhen('');
                        toast.success(
                          'Viewing booked — their details carried over.'
                        );
                      }}
