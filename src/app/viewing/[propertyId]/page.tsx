...
import { CalendarCheck, CheckCircle2, Home, Loader2, MapPin } from "lucide-react";
import {
  fetchPublicProperty, submitViewingRequest, type PublicProperty,
...
export default function PublicViewingPage() {
  ...
  const [done, setDone] = useState<{ reference: string; detail: string } | null>(null);
  ...
  const submit = async () => {
    ...
    try {
      ...
      setDone({ reference: res.reference, detail: res.detail });
    } catch (err) {
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          <CardContent className="pt-6 text-center space-y-2">
            ...
            <p className="font-medium text-slate-700">This listing isn't available</p>
      ...
      <div className="max-w-md mx-auto space-y-4">
        ...
        <div className="flex items-center justify-center gap-2 text-slate-900">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
            ...
            strokeLinejoin="round" className="text-teal-600">
        ...
        <Card className="overflow-hidden">
          ...
          <CardHeader className="pb-3">
            ...
            <p className="text-xs text-slate-500 pt-1">
              {property.category}{property.room_type ? ` · ${property.room_type}` : ""}
            </p>
          </CardHeader>
        </Card>
        ...
        {done ? (
          <Card>
            <CardContent className="pt-6 text-center space-y-3">
              ...
              <p className="text-xs text-slate-400">Reference: {done.reference}</p>
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <CalendarCheck className="h-4 w-4 text-teal-600" /> Request a viewing
              </CardTitle>
              <CardDescription>
                No account needed. Pick a time that works — the landlord will confirm or
                propose another time by email.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="250-555-0100" />
                </div>
              </div>
              ...
              <Button className="w-full bg-teal-600 hover:bg-teal-700"
                disabled={busy || !name || !email || !date} onClick={submit}>
