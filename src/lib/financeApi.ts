...
export type EntryType =
  | "RENT_CHARGE" | "UTILITY_CHARGE" | "DEPOSIT_CHARGE" | "FEE_CHARGE" | "OTHER_CHARGE"
  | "PAYMENT" | "CREDIT" | "EXPENSE" | "DEPOSIT_RETURN" | "REVERSAL"

export type ChargeStatus = "VOIDED" | "PAID" | "PARTIALLY_PAID" | "OVERDUE" | "DUE" | "SCHEDULED"

export type PaymentMethod = "ETRANSFER" | "CASH" | "CHEQUE" | "OTHER"

export type ExpenseCategory =
  | "MAINTENANCE" | "UTILITIES" | "INSURANCE" | "PROPERTY_TAX" | "MORTGAGE"
  | "STRATA" | "MANAGEMENT" | "SUPPLIES" | "ADVERTISING" | "OTHER"
...
export function leaseLabel(l: LeaseLite): string {
  ...
  if (l.start_date) {
    const end = l.is_month_to_month ? "Month-to-month" : (l.end_date ? fmt(l.end_date) : "Ongoing")
...
export async function fetchSummary(
  ...
  const res = await fetch(`${LEDGER}/summary/?${params}`, { headers: authHeaders(token, false) })
  return handle(res)
}
...
export async function fetchEntries(token: string, filters: EntryFilters = {}): Promise<LedgerEntry[]> {
  const params = new URLSearchParams()
  ...
  if (filters.entry_type__in) params.set("entry_type__in", filters.entry_type__in.join(","))
  if (filters.category) params.set("category", filters.category)
  if (filters.unpaid_only) params.set("paid_on__isnull", "true")
  if (filters.search) params.set("search", filters.search)
  if (filters.ordering) params.set("ordering", filters.ordering)

  const res = await fetch(`${LEDGER}/entries/?${params}`, { headers: authHeaders(token, false) })
  return unwrap<LedgerEntry>(await handle(res))
}

export async function fetchCharges(
  token: string,
  opts: { property?: number | string; status?: ChargeStatus; search?: string } = {},
): Promise<LedgerEntry[]> {
  const params = new URLSearchParams()
  if (opts.property) params.set("property", String(opts.property))
  ...
  const res = await fetch(`${LEDGER}/entries/charges/?${params}`, { headers: authHeaders(token, false) })
  return unwrap<LedgerEntry>(await handle(res))
}

export function fetchExpenses(
  token: string,
  opts: { property?: number | string; search?: string; unpaid_only?: boolean } = {},
): Promise<LedgerEntry[]> {
  return fetchEntries(token, { entry_type: "EXPENSE", ...opts, ordering: "-effective_date" })
}
 ...
 */
export async function downloadReceipt(token: string, paymentId: string): Promise<void> {
...
export async function recordPayment(
  token: string, chargeId: string, payload: RecordPaymentPayload,
): Promise<LedgerEntry> {
...
export async function voidEntry(token: string, id: string, reason: string): Promise<LedgerEntry> {
...
export async function correctEntry(
  token: string, id: string, changes: CorrectPayload,
): Promise<LedgerEntry> {
...
export async function postExpense(token: string, payload: ExpensePayload): Promise<LedgerEntry> {
...
export async function postCharge(token: string, payload: ChargePayload): Promise<LedgerEntry> {
...
export async function splitUtilityBill(
  token: string, payload: UtilityBillPayload,
): Promise<LedgerEntry[]> {
...
export async function addAttachment(
  token: string, id: string, file: File, label = "",
...
export async function fetchProperties(token: string): Promise<PropertyLite[]> {
  const res = await fetch(`${DJANGO_API_URL}/properties/`, { headers: authHeaders(token, false) })
  return unwrap<PropertyLite>(await handle(res))
}

export async function fetchLeasesLite(token: string): Promise<LeaseLite[]> {
  const res = await fetch(`${DJANGO_API_URL}/leases/`, { headers: authHeaders(token, false) })
  return unwrap<LeaseLite>(await handle(res))
}

export async function fetchLeaseTenants(token: string, leaseId: string): Promise<LeaseTenantLite[]> {
  const res = await fetch(`${DJANGO_API_URL}/leases/${leaseId}/`, { headers: authHeaders(token, false) })
...
export async function fetchLeaseBillingInfo(token: string, leaseId: string): Promise<LeaseBillingInfo> {
  const res = await fetch(`${DJANGO_API_URL}/leases/${leaseId}/`, { headers: authHeaders(token, false) })
  const data = await handle(res)
  return {
    id: data.id,
