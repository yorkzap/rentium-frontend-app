...
export async function fetchUnreadCount(token: string): Promise<number> {
  const res = await fetch(`${N}/unread_count/`, {
...
export async function markNotificationRead(
  ...
  const res = await fetch(`${N}/${id}/read/`, {
    method: 'POST',
...
export async function markAllNotificationsRead(token: string): Promise<number> {
  const res = await fetch(`${N}/read_all/`, {
    method: 'POST',
    headers: authHeaders(token),
  });
  const data = await handle(res);
...
export async function fetchMessages(
  ...
  const res = await fetch(`${MSG}/conversations/${conversationId}/messages/`, {
    headers: authHeaders(token, false),
  });
...
export async function fetchAgenda(
  ...
  const res = await fetch(`${AG}/?${params}`, {
    headers: authHeaders(token, false),
  });
  const data = await handle(res);
...
export async function deleteAgendaEvent(token: string, id: string) {
  const res = await fetch(`${AG}/events/${id}/`, {
    method: 'DELETE',
    headers: authHeaders(token, false),
  });
  return handle(res);
