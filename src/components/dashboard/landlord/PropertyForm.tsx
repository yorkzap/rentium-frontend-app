...
import {
  AlertTriangle, ArrowLeft, Building2, Camera, DoorOpen, Eye, EyeOff,
  Home, ImageIcon, Loader2, Sofa, Trash2, X,
} from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/contexts/AuthContext";
import {
  ApiError, BUILDING_AMENITIES, PropertyCategory, PropertyDetail,
  PropertyGroupStub, PropertyStatus, ROOM_TYPES, RoomType, STATUSES,
  UNIT_TYPES, UnitType,
  addGalleryImage, createProperty, deleteGalleryImage, fetchGroups,
  fetchProperty, updateProperty, uploadPrimaryImage,
} from "@/lib/propertyApi";
import AddressInput, { ResolvedAddress } from "@/components/form/AddressInput";
import {
  CardChoice, CheckboxRow, Field, NumberInput, Select, TextArea, TextInput,
...
const EMPTY: FormState = {
  name: "", description: "", property_category: "", unit_type: "", room_type: "",
  group_id: "", bedrooms: "", bathrooms: "", max_occupancy: "", square_footage: "",
  building_amenities: [], asking_rent: "", available_from: "",
  status: "AVAILABLE", is_publicly_visible: true, neighbourhood: "",
};
...
export default function PropertyForm({ propertyId }: Props) {
  ...
  useEffect(() => {
    if (!token) return;
    fetchGroups(token).then(setGroups).catch(() => setGroups([]));
  }, [token]);
  ...
  const validate = (): boolean => {
    ...
    if (form.name.trim().length < 3) e.name = "Give it a name people will recognise.";
    if (!form.property_category) e.property_category = "Pick one.";
    if (!address) e.address = "Pick an address from the list so we can find it on a map.";
  ...
  const submit = async () => {
    ...
    const body: Record<string, unknown> = {
      ...
      asking_rent: form.asking_rent ? Number(form.asking_rent).toFixed(2) : null,
    ...
    try {
      ...
      for (const f of galleryFiles) {
        try {
          ...
          toast.error(`Couldn't upload ${f.name} — you can add it from the property page.`);
        }
      }
      ...
      if (err instanceof ApiError) {
        ...
        toast.error(err.errors.detail ?? "Couldn't save — check the highlighted fields.");
      } else {
  ...
  return (
    <div className="mx-auto max-w-2xl pb-24">
      <PageHeader
        ...
        actions={
          <Link
            href={isEdit ? `/dashboard/properties/${propertyId}` : "/dashboard/properties"}
      ...
      {isEdit && existing && existing.publish_blockers.length > 0 && (
        <div className="card mb-6 border-[hsl(var(--warn))] bg-[hsl(var(--warn-soft))] p-4">
          <p className="flex items-center gap-2 text-sm font-medium text-[hsl(var(--warn-ink))]">
            <AlertTriangle className="h-4 w-4" /> This won't show up publicly yet
          </p>
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-4">
            <Field label="Name" required error={errors.name}
              ...
              <TextInput value={form.name} onChange={(v) => set("name", v)}
                         placeholder="e.g. McKenzie Room A" error={errors.name} />
            </Field>

            <Field label="What kind of place is it?" required error={errors.property_category}>
            ...
            <Field label="Description" hint="What makes it worth living in? This is what shows publicly.">
        ...
        <section className="card p-5">
          ...
          <AddressInput value={address} onChange={setAddress} error={errors.address} />
            ...
            <div className="space-y-4">
                ...
                <>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label="Bedrooms">
                      <NumberInput value={form.bedrooms} onChange={(v) => set("bedrooms", v)}
                                   min="0" step="1" placeholder="2" />
                    </Field>
                    <Field label="Bathrooms" hint="0.5 for a half bath">
                      <NumberInput value={form.bathrooms} onChange={(v) => set("bathrooms", v)}
                                   min="0" step="0.5" placeholder="1.5" />
                    </Field>
                  </div>
                          ...
                          onChange={(on) =>
                            set(
                              ...
                              on
                                ...
                                : form.building_amenities.filter((x) => x !== a.value),
                            )
                          }
                          label={a.label}
                        />
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="Max occupancy">
                  <NumberInput value={form.max_occupancy} onChange={(v) => set("max_occupancy", v)}
                               min="1" step="1" placeholder="2" />
                </Field>
                <Field label="Size">
                  <NumberInput value={form.square_footage} onChange={(v) => set("square_footage", v)}
                               min="1" step="1" placeholder="150" suffix="sq ft" />
                </Field>
              </div>
                ...
                <div className="flex items-start gap-3 rounded-lg bg-[hsl(var(--surface-sunken))] p-3">
                  ...
                  <div className="text-sm">
                    <p className="font-medium">
                      {existing.is_furnished ? "Listed as furnished" : "Listed as unfurnished"}
                    </p>
                    <p className="mt-0.5 text-xs text-[hsl(var(--ink-4))]">
                      We work this out from your inventory — a room counts as furnished
                      once there's something to sleep on in it.{" "}
                      <Link href="/dashboard/inventory" className="text-[hsl(var(--brand))] hover:underline">
                        Manage inventory
                      </Link>
                    </p>
                  </div>
        ...
        <section className="card p-5">
          ...
          <p className="mb-4 mt-1 text-sm text-[hsl(var(--ink-4))]">
            What you're advertising it for. This isn't the rent on any lease — a
            vacant place has no lease, so without this there'd be nothing to show
            someone looking.
          </p>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Asking rent" error={errors.asking_rent} hint="Per month">
              <NumberInput value={form.asking_rent} onChange={(v) => set("asking_rent", v)}
                           min="0" step="0.01" prefix="$" placeholder="1200"
        ...
        <section className="card p-5">
          ...
          <p className="mb-4 mt-1 text-sm text-[hsl(var(--ink-4))]">
            The first photo does most of the work. Nobody enquires about a grey box.
          </p>
          ...
          <div className="space-y-4">
            <div>
              ...
              {primaryPreview ? (
                ...
                <label className="flex aspect-[16/10] cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed transition-colors hover:bg-[hsl(var(--surface-sunken))]"
                       style={{ borderColor: "hsl(var(--line-strong))" }}>
                  <Camera className="mb-2 h-7 w-7 text-[hsl(var(--ink-5))]" />
                  <span className="text-sm font-medium">Add the main photo</span>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                {galleryFiles.map((f, i) => (
                  <div key={i} className="relative aspect-square overflow-hidden rounded-lg bg-[hsl(var(--surface-sunken))]">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                ...
                <label className="flex aspect-square cursor-pointer items-center justify-center rounded-lg border-2 border-dashed transition-colors hover:bg-[hsl(var(--surface-sunken))]"
                       style={{ borderColor: "hsl(var(--line-strong))" }}>
        ...
        <section className="card p-5">
          ...
          <p className="mb-4 mt-1 text-sm text-[hsl(var(--ink-4))]">
            This mostly looks after itself — activating a lease marks it Occupied,
            starting maintenance marks it Under maintenance — but anything you set
            by hand always wins.
          </p>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t bg-white/95 backdrop-blur md:pl-[232px]"
           style={{ borderColor: "hsl(var(--line))" }}>
        <div className="mx-auto flex max-w-2xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <p className="text-xs text-[hsl(var(--ink-4))]">
            {form.status === "AVAILABLE" && form.is_publicly_visible && form.asking_rent && address ? (
              <span className="flex items-center gap-1.5">
                <Eye className="h-3.5 w-3.5" /> Will appear publicly (if your page is on)
              </span>
            ) : (
