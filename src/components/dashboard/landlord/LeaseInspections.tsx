...
import { AlertTriangle, ClipboardCheck, Loader2, Plus, ChevronLeft } from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/contexts/AuthContext";
import { fetchLeaseDetails } from "@/lib/leaseApi";
import {
  InspectionSummary, createInspection, fetchInspections,
...
export default function LeaseInspections({ leaseId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setIsRoomScope(
        String(lease.lease_type || "").includes("ROOMMATE") || !!lease.group_name
      );
    } catch (err) {
  ...
  const missingTenants = useMemo(() => {
    ...
    return leaseTenants.filter((lt) => lt.tenant && !lt.declined && !covered.has(lt.id));
  }, [isRoomScope, inspections, leaseTenants]);

  const missingUnitInspection = !isRoomScope && inspections.length === 0;
  const showNag = leaseStatus === "ACTIVE" && (missingTenants.length > 0 || missingUnitInspection);
  ...
  const handleCreate = async (leaseTenantId: string | null) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Couldn't create inspection.");
    } finally {
      setIsCreating(null);
    }
  };
  ...
  if (openInspectionId) {
    return (
      <div className="space-y-3">
        <Button variant="ghost" size="sm" onClick={() => { setOpenInspectionId(null); load(); }}>
        ...
        <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-md">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-amber-800">
              <p className="font-medium">Move-in condition inspection missing</p>
              <p className="mt-0.5">
                {isRoomScope
                ...
                In BC, doing a joint walk-through on the possession day and recording it
                (RTB-27) is what preserves your right to claim against the deposit for
                damage. Nothing is blocked — but it's strongly recommended.
              </p>
            </div>
          </div>
        </div>
        ...
        <CardHeader className="pb-3 flex flex-row items-start justify-between flex-wrap gap-2">
          <div>
            <CardTitle className="flex items-center text-lg">
              <ClipboardCheck className="h-5 w-5 mr-2 text-slate-600" /> Condition Inspections
            </CardTitle>
            <CardDescription>Move-in / move-out condition reports (RTB-27 style)</CardDescription>
          </div>
          <div className="flex gap-2 flex-wrap">
            {isRoomScope ? (
              missingTenants.map((lt) => (
                <Button
                  key={lt.id} size="sm" variant="outline"
              ...
              <Button size="sm" disabled={isCreating !== null} onClick={() => handleCreate(null)}>
                {isCreating === "__unit"
                  ? <Loader2 className="h-4 w-4 mr-1 animate-spin" />
                  : <Plus className="h-4 w-4 mr-1" />}
        ...
        <CardContent className="p-0">
          {inspections.length === 0 ? (
            <p className="p-6 text-center text-sm text-slate-500">
              No inspections yet. Create one above and walk the unit together on the possession day.
            </p>
          ) : (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        ...
                        {insp.move_in_inspection_date ? ` · Move-in inspected ${insp.move_in_inspection_date}` : ""}
                        {insp.move_out_inspection_date ? ` · Move-out inspected ${insp.move_out_inspection_date}` : ""}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${STATUS_BADGE[insp.status] || "bg-slate-100 text-slate-700"}`}>
                        {insp.status_display}
                      </span>
                    </div>
                  </button>
