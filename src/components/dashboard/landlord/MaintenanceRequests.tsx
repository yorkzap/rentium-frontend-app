...
import {
  Wrench, AlertCircle, CheckCircle, Clock, Plus, Search, Loader2, ShieldAlert,
  Flame, CalendarClock, MessageSquare, Send,
} from "lucide-react"
import { useAuth } from "@/contexts/AuthContext"
import { toast } from "sonner"
import {
  fetchWorkOrders, createWorkOrder, updateWorkOrder, transitionWorkOrder,
  completeWorkOrder, addComment, fetchAreas, CATEGORIES, PRIORITIES,
  type WorkOrder, type WorkOrderStatus, type WorkOrderCategory, type WorkOrderPriority, type Area,
...
function slaLabel(wo: WorkOrder): { text: string; cls: string } | null {
  ...
  if (wo.sla_breached) return { text: "SLA breached", cls: "bg-red-50 text-red-700" }
...
export default function MaintenanceRequests() {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      const data = await fetchWorkOrders(token, {
        status: statusFilter === "all" ? undefined : (statusFilter as WorkOrderStatus),
  ...
  useEffect(() => { if (token) fetchProperties(token).then(setProperties).catch(() => {}) }, [token])
    ...
    <div className="space-y-6">
      ...
      {breaches > 0 && (
        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-md flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-red-800">{breaches} work order(s) past their response deadline</h3>
            <p className="text-sm text-red-700 mt-1">Emergency repairs under BC's RTA must be addressed promptly. Schedule or start these now.</p>
          </div>
        </div>
      )}

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <MiniStat label="New / unactioned" value={counts.NEW} icon={<AlertCircle className="h-5 w-5" />} tone="amber" />
        <MiniStat label="In progress" value={counts.IN_PROGRESS} icon={<Clock className="h-5 w-5" />} tone="blue" />
        <MiniStat label="Completed" value={counts.COMPLETED} icon={<CheckCircle className="h-5 w-5" />} tone="green" />
      </div>

      <div className="flex justify-between items-center gap-3">
        <h2 className="text-lg font-medium">Work Orders</h2>
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-[170px]"><SelectValue placeholder="Any status" /></SelectTrigger>
          <SelectContent>
      ...
      {createOpen && (
        <CreateWorkOrderDialog
          ...
          onClose={() => setCreateOpen(false)} onDone={() => { setCreateOpen(false); reload() }}
        />
      )}
      {manage && (
        <ManageWorkOrderDialog
          token={token!} workOrder={manage}
          onClose={() => setManage(null)} onChanged={reload}
...
function MiniStat({ label, value, icon, tone }: {
  label: string; value: number; icon: React.ReactNode; tone: "amber" | "blue" | "green"
}) {
  const tones = { amber: "bg-amber-50 text-amber-600", blue: "bg-blue-50 text-blue-600", green: "bg-green-50 text-green-600" }
  return (
    <Card>
...
// -------------------------------------------------------------- create
function CreateWorkOrderDialog({ token, properties, onClose, onDone }: {
  token: string; properties: PropertyLite[]; onClose: () => void; onDone: () => void
}) {
  ...
  useEffect(() => {
    setArea("none"); setAreas([])
    if (property && token) fetchAreas(token, property).then(setAreas).catch(() => {})
  }, [property, token])
    ...
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Log an issue or schedule preventive work on one of your properties.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Select value={area} onValueChange={setArea} disabled={!property}>
                <SelectTrigger><SelectValue placeholder="Whole unit" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Whole unit</SelectItem>
                  {areas.map((a) => <SelectItem key={a.id} value={a.id}>{a.name} · {a.kind_display}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          <div className="space-y-2">
            <Label>Title</Label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Furnace not igniting" />
          </div>
          <div className="space-y-2">
...
// -------------------------------------------------------------- manage
function ManageWorkOrderDialog({ token, workOrder, onChanged, onClosedDialog }: {
  token: string; workOrder: WorkOrder; onClose: () => void; onChanged: () => void; onClosedDialog: () => void
}) {
  ...
  const refreshLocal = (next: WorkOrder) => { setWo(next); onChanged() }
  ...
  const saveDetails = async () => {
    setBusy("details")
    try {
      refreshLocal(await updateWorkOrder(token, wo.id, {
  ...
  const doComplete = async () => {
    setBusy("complete")
    try {
      refreshLocal(await completeWorkOrder(token, wo.id, {
        cost: cost || undefined, post_expense: postExpense && !!cost, vendor: contractor,
      }))
      toast.success(postExpense && cost ? "Completed and expense booked." : "Completed.")
    } catch (err) {
  ...
  const otherTransitions = wo.allowed_transitions.filter((s) => s !== "COMPLETED")
    ...
    <Dialog open onOpenChange={(o) => !o && onClosedDialog()}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {wo.property_name}{wo.area_name ? ` · ${wo.area_name}` : ""} · {wo.category_display} · {wo.priority_display}
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-3 border rounded-md p-3 bg-slate-50">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input value={contractor} onChange={(e) => setContractor(e.target.value)} placeholder="Name" />
                  </div>
                </div>
                <Button variant="outline" size="sm" onClick={saveDetails} disabled={busy === "details"}>
                  {busy === "details" && <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" />} Save details
                </Button>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {otherTransitions.map((s) => (
                    <Button
                      ...
                      onClick={() => doTransition(s)} disabled={!!busy}
                      className={s === "CANCELLED" ? "text-red-600 border-red-200 hover:bg-red-50" : ""}
                    >
                      {busy === s && <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" />}
                      {s === "SCHEDULED" ? "Mark scheduled" : s === "IN_PROGRESS" ? "Start work" : s === "NEW" ? "Reopen" : "Cancel"}
                    </Button>
                  ))}
                </div>
              )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder="Add a note…" onKeyDown={(e) => e.key === "Enter" && sendComment()}
              />
              <Button variant="outline" size="icon" onClick={sendComment} disabled={busy === "comment" || !comment.trim()}>
        ...
        <DialogFooter>
          <Button variant="ghost" onClick={onClosedDialog}>Close</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
