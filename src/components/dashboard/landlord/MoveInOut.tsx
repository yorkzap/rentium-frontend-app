    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Upcoming Move-Ins</p>
              ...
              {moveIns.map((moveIn) => (
                <div key={moveIn.id} className="p-2 bg-slate-50 rounded-md text-sm">
                  <div className="flex justify-between">
                    ...
                    <div
                      className={`text-xs px-2 py-1 rounded-full ${
                        moveIn.status === "Scheduled" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"
                      }`}
                    >
                      {moveIn.status}
                    </div>
                  ...
                  <div className="flex justify-between mt-2 text-xs">
                    ...
                    <div className="flex items-center">
                      <CheckSquare className="h-3 w-3 mr-1" /> Deposit: {moveIn.depositPaid ? "Paid" : "Pending"}
                    </div>
                  </div>
                </div>
              ))}
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Upcoming Move-Outs</p>
                <p className="text-2xl font-bold">2</p>
              </div>
              ...
              {moveOuts.map((moveOut) => (
                <div key={moveOut.id} className="p-2 bg-slate-50 rounded-md text-sm">
                  <div className="flex justify-between">
                    ...
                    <div
                      className={`text-xs px-2 py-1 rounded-full ${
                        moveOut.status === "Scheduled" ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"
                      }`}
                    >
                      {moveOut.status}
                    </div>
                  ...
                  <div className="flex justify-between mt-2 text-xs">
                    ...
                    <div className="flex items-center">
                      <CheckSquare className="h-3 w-3 mr-1" /> Inspection: {moveOut.inspectionDate}
                    </div>
                  </div>
                </div>
              ))}
      ...
      <Tabs defaultValue="moveins">
          ...
          <Card>
            ...
            <CardContent>
              <div className="rounded-md border">
                ...
                {moveIns.map((moveIn) => (
                  <div key={moveIn.id} className="grid grid-cols-6 border-t p-3 text-sm">
                    <div className="font-medium">{moveIn.tenant}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="rounded-md border">
                ...
                {moveOuts.map((moveOut) => (
                  <div key={moveOut.id} className="grid grid-cols-7 border-t p-3 text-sm">
                    <div className="font-medium">{moveOut.tenant}</div>
        ...
        <TabsContent value="checklists" className="mt-4">
          <Card>
            <CardContent className="p-6">
              <h3 className="text-lg font-medium mb-4">Move-In/Out Checklists</h3>
              <p className="text-muted-foreground mb-4">
                Standardized checklists for property condition assessment during move-in and move-out.
              </p>
