...
export default function LeaseAppointments({
  ...
  const load = useCallback(async () => {
    if (!token) return;
    try {
      setItems(await listAppointments(token, { lease: leaseId }));
    } catch {
      /* appointments app may not be wired yet */
    } finally {
      setLoading(false);
    }
  }, [token, leaseId]);
  ...
  const create = async () => {
    ...
    try {
      ...
      toast.success(
        'Scheduled — the tenant(s) are notified. In BC, entry needs 24h written notice; this record is that notice.'
      );
      setShowForm(false);
      setWhen('');
      setWho('');
  ...
  const cancel = async (id: string) => {
    ...
    try {
      ...
      toast.success('Cancelled — the tenant(s) are notified.');
      load();
    } catch (err) {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          <CalendarClock className="h-4 w-4 text-slate-500" /> Appointments &
          Viewings
        </CardTitle>
        <CardDescription>
          Showings for the next tenant and contractor visits. Tenants on this
          lease see every scheduled visit — that&apos;s their written entry
          notice (24h in BC).
        </CardDescription>
      </CardHeader>
            ...
            {upcoming.map((a) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <Badge variant="outline" className={kindBadge(a.kind)}>
                    {a.kind_display}
                  </Badge>
                  <Button
                    variant="ghost"
          ...
          <div className="rounded-md border p-3 space-y-3 text-sm">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  value={kind}
                  onValueChange={(v) => setKind(v as AppointmentKind)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="Prospective tenant / ABC Plumbing"
                />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="e.g. accessing the kitchen and bathroom only"
              />
            </div>
        ...
        {past.length > 0 && (
          <p className="text-xs text-slate-400">
            ...
            {past
              .map(
                (a) =>
                  `${a.kind_display} ${fmtDT(a.starts_at)} (${a.status_display})`
              )
              .join(' · ')}
          </p>
        )}
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Your landlord has scheduled the following — this listing is your
