...
import {
  Loader2, AlertCircle, ArrowLeft, Mail, CheckCircle2, Clock, Signature, XCircle, Copy,
  UserPlus, Trash2, Ban, FileDown,
} from "lucide-react"
...
export default function LeaseDetail({ leaseId }: { leaseId: string }) {
  ...
  const fetchLease = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Unknown error loading lease"
      setError(msg)
      toast.error(msg)
    } finally {
      setIsLoading(false)
  ...
  const handleLandlordSign = async () => {
    ...
    setIsSigning(true)
    try {
      const res = await fetch(`${DJANGO_API_URL}/leases/${leaseId}/landlord_sign/`, {
        method: "POST",
        headers: { Authorization: `Token ${token}` },
      })
      if (!res.ok) {
      ...
      toast.success("Lease signed. It will activate once all tenants have signed too.")
      fetchLease()
    } catch (err) {
  ...
  const handleResendInvite = async (leaseTenantId: string) => {
    ...
    try {
      const res = await fetch(`${DJANGO_API_URL}/leases/tenants/${leaseTenantId}/resend_invite/`, {
        method: "POST",
        headers: { Authorization: `Token ${token}` },
      })
      if (!res.ok) {
        const body = await res.json().catch(() => ({}))
        throw new Error(body.detail || `Failed to resend invite (${res.status})`)
      }
  ...
  const handleCopyInviteLink = async (inviteUrl: string) => {
    try {
      ...
      toast.success("Invite link copied — it's live until the tenant uses it to set a password.")
    } catch {
      ...
      try {
        ...
        setRosterRows((prev) =>
          prev.map((row, i) => {
            ...
            return { ...row, rentAmount: resultRow.rent_amount, hasSigned: resultRow.has_signed }
          })
        )
      } catch (err) {
        toast.error(err instanceof Error ? err.message : "Couldn't recalculate the rent split.")
      } finally {
  ...
  const openRosterEditor = () => {
    ...
    const withNewRow = [...existingRows, { id: null, email: "", name: "", linked: false, rentAmount: "", isPrimary: existingRows.length === 0, touched: false, hasSigned: false }]
  ...
  const addRosterRow = () => {
    setRosterRows((prev) => {
      const next = [...prev, { id: null, email: "", name: "", linked: false, rentAmount: "", isPrimary: false, touched: false, hasSigned: false }]
      syncSplit(next)
      return next
    })
  }
  const removeRosterRow = (index: number) => {
    setRosterRows((prev) => {
      ...
      if (row.id) {
        toast.error("Existing tenants can't be removed here — use Django admin, or Terminate the lease.")
        return prev
      }
  ...
  const updateRosterEmail = (index: number, email: string) => {
    setRosterRows((prev) => prev.map((row, i) => (i === index ? { ...row, email } : row)))
  }
  const updateRosterName = (index: number, name: string) => {
    setRosterRows((prev) => prev.map((row, i) => (i === index ? { ...row, name } : row)))
  }
  const updateRosterRent = (index: number, value: string) => {
    setRosterRows((prev) => {
      const next = prev.map((row, i) => (i === index ? { ...row, rentAmount: value, touched: true } : row))
  ...
  const setRosterPrimary = (index: number) => {
    setRosterRows((prev) => prev.map((row, i) => ({ ...row, isPrimary: i === index })))
  }
  const rosterAllocatedTotal = rosterRows.reduce((sum, r) => sum + (parseFloat(r.rentAmount) || 0), 0)
  const rosterUnallocated = lease ? Math.max(Number(lease.total_rent) - rosterAllocatedTotal, 0) : 0
  const handleSaveRoster = async () => {
    if (!token || !lease) return
    const originalById = new Map(lease.lease_tenants.map((lt) => [lt.id, lt]))
    const changedExisting = rosterRows.filter(
      (r) => r.id && !r.hasSigned && r.rentAmount !== originalById.get(r.id)?.rent_amount
    )
    ...
    const newRows = rosterRows.filter((r) => !r.id && r.email.trim() && r.rentAmount)
    if (changedExisting.length === 0 && primaryChangedExisting.length === 0 && nameChangedExisting.length === 0 && newRows.length === 0) {
    ...
    try {
      ...
      for (const r of changedExisting) {
        updatesById.set(r.id as string, { ...(updatesById.get(r.id as string) || {}), rent_amount: Number(r.rentAmount) })
      }
      for (const r of primaryChangedExisting) {
        updatesById.set(r.id as string, { ...(updatesById.get(r.id as string) || {}), is_primary_tenant: r.isPrimary })
      }
      for (const r of nameChangedExisting) {
        updatesById.set(r.id as string, { ...(updatesById.get(r.id as string) || {}), invited_name: r.name.trim() })
      }
      const updatePromises = Array.from(updatesById.entries()).map(([id, body]) =>
        fetch(`${DJANGO_API_URL}/leases/tenants/${id}/`, { method: "PATCH", headers, body: JSON.stringify(body) })
      )
      ...
      toast.error(err instanceof Error ? err.message : "Failed to save tenant roster.")
    } finally {
  ...
  const handleDeleteDraft = async () => {
    ...
    try {
      ...
      if (!res.ok && res.status !== 204) {
        const body = await res.json().catch(() => ({}))
        throw new Error(body.detail || `Failed to delete draft (${res.status})`)
      }
  ...
  const handleTerminate = async () => {
    ...
    try {
      const res = await fetch(`${DJANGO_API_URL}/leases/${lease.id}/terminate/`, {
        method: "POST",
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}))
        throw new Error(body.detail || `Failed to terminate lease (${res.status})`)
      }
    ...
    return (
      <Card className="border-destructive bg-red-50">
        <CardContent className="p-6 text-center">
          ...
          <p className="text-destructive font-medium">{error || "Lease not found."}</p>
          <Button variant="outline" className="mt-4" onClick={() => router.push("/dashboard/leases")}>
            Back to Leases
          </Button>
        </CardContent>
      </Card>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => router.push("/dashboard/leases")}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {lease.property_address || lease.lease_number} · {lease.lease_type_display}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-medium ${STATUS_STYLES[lease.status] || "bg-slate-100 text-slate-700"}`}>
          ...
          <Button variant="outline" size="sm" onClick={handleDownloadPdf} disabled={isDownloadingPdf}>
            {isDownloadingPdf ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : <FileDown className="h-4 w-4 mr-1" />}
            Download PDF
          </Button>
          {lease.status === "DRAFT" && (
            <Button
      ...
      {hasDeclinedTenants && (
        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-md flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-red-800">A tenant declined this lease</h3>
            <p className="text-sm text-red-700 mt-1">
              Check the reason below (if provided) and either remove that tenant slot to invite
              someone else, or terminate this lease if it's no longer moving forward.
            </p>
          </div>
        </div>
      )}
      {!lease.landlord_signed && !lease.is_locked && (
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-md flex items-center justify-between gap-4">
          <div className="flex items-start">
            ...
            <div>
              <h3 className="text-sm font-medium text-blue-800">Your signature is needed</h3>
          ...
          <Button size="sm" onClick={handleLandlordSign} disabled={isSigning}>
            {isSigning ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Signature className="h-4 w-4 mr-2" />}
            Sign as Landlord
          </Button>
        </div>
      )}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2">
          ...
          <CardContent className="grid grid-cols-2 gap-4 text-sm">
            <div><p className="text-slate-500">Lease Number</p><p className="font-mono text-xs pt-0.5">{lease.lease_number}</p></div>
            <div><p className="text-slate-500">Agreement</p><p>{lease.lease_type_display}</p></div>
            <div><p className="text-slate-500">Start Date</p><p>{lease.start_date}</p></div>
            <div><p className="text-slate-500">End Date</p><p>{lease.is_month_to_month ? "Month-to-month" : lease.end_date || "—"}</p></div>
            {lease.move_in_date && lease.move_in_date !== lease.start_date && (
              <div><p className="text-slate-500">Move-in Date</p><p>{lease.move_in_date}</p></div>
            )}
            <div><p className="text-slate-500">Total Monthly Rent</p><p className="font-medium">${lease.total_rent}</p></div>
            <div><p className="text-slate-500">Security Deposit</p><p>${lease.security_deposit}</p></div>
            <div><p className="text-slate-500">Pet Deposit</p><p>${lease.pet_deposit}</p></div>
            <div><p className="text-slate-500">Cleaning Fee</p><p>${lease.cleaning_fee}</p></div>
            {Number(lease.unallocated_rent) > 0.01 && (
              <div><p className="text-slate-500">Unallocated</p><p className="font-medium text-amber-600">${lease.unallocated_rent}</p></div>
            )}
            <div><p className="text-slate-500">Pets</p><p>{lease.pets_allowed ? "Allowed" : "Not allowed"}</p></div>
            <div><p className="text-slate-500">Smoking</p><p>{lease.smoking_allowed ? "Allowed" : "Not allowed"}</p></div>
            {(lease.effective_etransfer_email || lease.etransfer_email) && (
              <div className="col-span-2 pt-2 border-t">
                <p className="text-slate-500 mb-1">e-Transfers go to</p>
                <p className="select-all">{lease.etransfer_email || lease.effective_etransfer_email}</p>
                {!lease.etransfer_email && (
                  <p className="text-xs text-slate-400 mt-0.5">
                    Fallback (your service/account email) — set a dedicated e-transfer address when creating a lease.
                  </p>
                )}
              </div>
            )}
            {lease.bills_summary && (
              <div className="col-span-2 pt-2 border-t"><p className="text-slate-500 mb-1">Bills</p><p>{lease.bills_summary}</p></div>
            )}
            {lease.common_space_clause_text && (
              <div className="col-span-2 pt-2 border-t"><p className="text-slate-500 mb-1">Shared Spaces</p><p>{lease.common_space_clause_text}</p></div>
            )}
            {lease.special_terms && (
              <div className="col-span-2 pt-2 border-t"><p className="text-slate-500 mb-1">Special Terms</p><p className="whitespace-pre-wrap">{lease.special_terms}</p></div>
            )}
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            {lease.landlord_signed ? (
              <div className="flex items-center text-green-700 text-sm">
                <CheckCircle2 className="h-4 w-4 mr-2" /> Signed {lease.landlord_signed_date ? new Date(lease.landlord_signed_date).toLocaleDateString() : ""}
              </div>
            ) : (
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            ...
            <CardDescription>Invite and signature status for everyone on this lease</CardDescription>
          </div>
          {!lease.is_locked && !isAddTenantOpen && (
            <Button variant="outline" size="sm" onClick={openRosterEditor}>
              <UserPlus className="h-4 w-4 mr-1" /> {lease.lease_tenants.length === 0 ? "Add Tenant" : "Manage Tenants"}
            </Button>
          )}
        </CardHeader>
        <CardContent className="space-y-3">
          {isAddTenantOpen && (
            <div className="border rounded-md p-4 bg-slate-50 space-y-3">
              <p className="text-xs text-slate-500 flex items-center gap-1.5">
                Total monthly rent is <strong>${lease.total_rent}</strong> — already-signed tenants
                (locked, greyed out) keep their agreed amount; editing any other row auto-adjusts
                the rest so the total stays correct. The full legal name goes on the lease form
                (e.g. the RTB-1 parties and signature blocks).
                {isSyncingSplit && <Loader2 className="h-3 w-3 animate-spin text-slate-400" />}
              </p>
              {rosterRows.map((row, index) => (
                <div key={index} className={`flex items-end gap-2 ${row.hasSigned ? "opacity-60" : ""}`}>
                  ...
                  {!row.id && (
                    <Button variant="ghost" size="icon" className="mb-0.5" onClick={() => removeRosterRow(index)}>
              ...
              {rosterUnallocated > 0.01 && (
                <p className="text-xs text-amber-600">
                  ${rosterUnallocated.toFixed(2)} still unassigned — the lease can't be signed until
                  this reaches $0.
                </p>
              )}
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="ghost" size="sm" onClick={() => setIsAddTenantOpen(false)} disabled={isAddingTenant}>
                  Cancel
                </Button>
                <Button size="sm" onClick={handleSaveRoster} disabled={isAddingTenant}>
                  {isAddingTenant ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : null}
                  Save
                </Button>
              </div>
            </div>
          ...
          {lease.lease_tenants.length === 0 && !isAddTenantOpen && (
            <p className="text-sm text-slate-500 py-4 text-center">
              No tenants added yet. Click "Add Tenant" above to invite the first one.
            </p>
          )}
          {lease.lease_tenants.map((lt) => (
            <div key={lt.id} className={`border rounded-md p-4 ${lt.declined ? "border-red-200 bg-red-50/40" : ""}`}>
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium text-sm flex items-center gap-2">
                    {lt.tenant_name || lt.invited_name || lt.invited_email}
                    {lt.is_primary_tenant && <Badge variant="outline" className="text-xs">Primary</Badge>}
                  </div>
                  <div className="text-xs text-slate-500 mt-0.5">
                    {lt.tenant_email || lt.invited_email} · ${lt.rent_amount}/mo
                    {lt.effective_rent !== lt.rent_amount && (
                      <span className="text-amber-600"> (effective ${lt.effective_rent})</span>
                    )}
                    {lt.room_name && ` · ${lt.room_name}`}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${INVITE_STATUS_STYLES[lt.invite_status]}`}>
                    {lt.invite_status}
                  </span>
                  {lt.declined ? (
                    <span className="inline-flex items-center text-xs text-red-700">
                      <XCircle className="h-3.5 w-3.5 mr-1" /> Declined
                      {lt.declined_at && ` ${new Date(lt.declined_at).toLocaleDateString()}`}
                    </span>
                  ) : lt.has_signed ? (
                  ...
                  {!lt.declined && lt.invite_url && (
                    <Button
                      ...
                      onClick={() => handleCopyInviteLink(lt.invite_url as string)}
                    >
                  ...
                  {!lt.declined && lt.invite_status !== "LINKED" && (
                    ...
                    >
                      {resendingId === lt.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Mail className="h-3.5 w-3.5" />}
                    </Button>
                  )}
                </div>
              </div>
      ...
      {lease.status !== "DRAFT" && (
        <LeaseMoveOuts leaseId={lease.id} isActive={lease.status === "ACTIVE"} />
      )}
      {/* Viewings & contractor visits; tenants on the lease see them too. */}
      {lease.status !== "DRAFT" && (
        <LeaseAppointments
          leaseId={lease.id}
          propertyId={(lease as unknown as { property?: number | null }).property ?? null}
        />
      )}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently deletes lease {lease.lease_number} and any tenant invites on it.
              This can't be undone. Only draft leases can be deleted this way — once tenants
              start signing, use "Terminate" instead so there's a record of what happened.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={terminateDialogOpen} onOpenChange={setTerminateDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This ends lease {lease.lease_number} effective today. The record is kept (not
              deleted) so there's a history of the tenancy. This can't be undone from here —
              contact support or use Django admin if you need to reverse it.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              className="bg-red-600 hover:bg-red-700"
            >
              {isTerminating ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : null}
              Terminate Lease
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
