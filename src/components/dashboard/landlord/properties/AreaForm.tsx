...
// Input vs output diverge (`coerce` takes strings in; `.default` makes the
// array optional in) — so the form runs on the INPUT shape and submit
// handlers receive the parsed OUTPUT shape.
type AreaFormInput = z.input<typeof areaFormSchema>;
type AreaFormData = z.output<typeof areaFormSchema>;
...
export default function AreaForm({
  ...
  const form = useForm<AreaFormInput, unknown, AreaFormData>({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const existingShared = (areaToEdit?.shared_by ?? []).filter(
        (id) => id !== propertyId
      );
      form.reset({
  ...
  const handleFormSubmit = async (data: AreaFormData) => {
    ...
    const shared_by =
      data.shared_with_ids.length > 0
        ? [propertyId, ...data.shared_with_ids]
        : [];
      ...
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditMode
              ? 'Update details.'
              : 'Define an area for this property.'}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
          ...
          >
            ...
            {canShare && (
              <FormField
                control={form.control}
                name="shared_with_ids"
                render={({ field }) => (
                  <FormItem className="space-y-2">
                    <FormLabel className="flex items-center gap-1.5">
                      <Users className="h-4 w-4" /> Share with rooms in this
                      group
                    </FormLabel>
                    <FormDescription>
                      Leave all unchecked to keep this area private to this
                      room.
                    </FormDescription>
                    <FormControl>
                      <div className="space-y-1 rounded-md border p-3">
                        {groupSiblings.map((sibling) => {
                          ...
                          const checked = selected.includes(sibling.id);
                          return (
                            <div
                              key={sibling.id}
            ...
            {!canShare && isRoom && (
              <p className="text-xs text-muted-foreground border rounded-md p-3">
                This room is not in a group (or has no grouped siblings), so
                this area will be private to this room.
              </p>
            )}
