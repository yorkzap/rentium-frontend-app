...
import { cn, capitalize } from '@/lib/utils'; // Import utils
...
try {
  ...
  if (
    ...
    console.warn(
      'NEXT_PUBLIC_DJANGO_API_URL is not set or invalid in PropertyList. Image URLs might be incorrect.'
    );
  }
} catch (e) {
...
const getFullImageUrl = (
  ...
  if (djangoBaseUrl) {
    try {
      ...
      console.error(
        `Error constructing URL for ${relativeUrl} with base ${djangoBaseUrl}:`,
        e
      );
      return null;
    }
  ...
  console.warn(
    'Cannot construct full image URL in PropertyList: djangoBaseUrl could not be derived.'
  );
  return relativeUrl;
};
...
export function PropertyList({
  ...
  // NOTE: must run before the empty-state early return below — hooks can't
  // be called conditionally, and this one used to sit after it.
    ...
    return (
      <Card className="mt-4 border-dashed border-line-strong bg-canvas/60 col-span-1 md:col-span-2 xl:col-span-3">
        <CardContent className="p-8 text-center flex flex-col items-center">
          ...
          <h3 className="text-xl font-medium text-ink-2 mb-1">
            No Properties Found
          </h3>
  ...
  return (
    <div className="space-y-6">
      ...
      {Array.from(propertiesByGroup.entries())
        ...
        .map(([key, properties]) => {
          const groupName =
            properties[0]?.group?.name ?? properties[0]?.group_name;
  ...
  function renderPropertyCard(property: PropertyDetail) {
    ...
    const categoryDisplay =
      ...
      formatCategory(property.property_category);
    const statusDisplay =
      property.status_display || formatStatus(property.status);

    // Support both type_display directly from API and derived values from type fields
    const typeDetailDisplay =
      property.type_display ||
      (property.property_category === 'COMPLETE_UNIT'
        ? property.unit_type_display ||
          (property.unit_type
            ? capitalize(property.unit_type.replace('_', ' '))
            : '-')
        : property.room_type_display ||
          (property.room_type
            ? capitalize(property.room_type.replace('_', ' '))
            : '-'));
    ...
    return (
      <div
        ...
        className={cn(
          'cursor-pointer group rounded-lg border bg-card text-card-foreground shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col overflow-hidden h-full'
        )}
      ...
      >
        <div className="relative bg-surface-sunken border-b w-full h-80 overflow-hidden">
          ...
          <div className="absolute top-2 left-2 flex flex-col gap-1.5 items-start z-10">
            <Badge
              variant="outline"
              className={`text-xs px-1.5 py-0.5 ${getCategoryColor(property.property_category)} backdrop-blur-sm bg-white/70 shadow-sm`}
            >
              {' '}
              {categoryDisplay}{' '}
            </Badge>
            <Badge
              variant="outline"
              className={`text-xs px-1.5 py-0.5 ${getStatusColor(property.status)} backdrop-blur-sm bg-white/70 shadow-sm`}
            >
              {' '}
              {statusDisplay}{' '}
            </Badge>
        ...
        <div className="p-4 flex-1 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-1">
              <h3
                className="font-semibold text-lg leading-tight truncate pr-2"
                title={property.name || ''}
              >
                {' '}
                {property.name || '-'}{' '}
              ...
              <DropdownMenu>
                ...
                >
                  ...
                  <DropdownMenuItem
                    ...
                    onClick={(e) => handleDeleteRequest(e, property)}
                  >
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-auto pt-3 border-t border-line">
            ...
            {property.area_summary &&
              property.area_summary !== 'No areas defined' && (
                <div className="col-span-2 mt-1">
                  <DetailItem
                    icon={Info}
                    value={property.area_summary}
                    label="Areas"
                  />
                </div>
              )}
