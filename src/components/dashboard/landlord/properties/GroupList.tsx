  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center">
        ...
        <p className="text-slate-500">Loading property groups...</p>
      </div>
    );
  }
      ...
      {groups.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-sm text-slate-500 mb-4">Create groups to manage rooms with shared areas.</p>
              ...
              <Card key={group.id} className="overflow-hidden hover:shadow-md transition-shadow flex flex-col h-full">
                ...
                <CardContent className="py-3 flex-grow">
                  <div className="space-y-4">
                    {/* Properties Section */}
                    <div>
                      <h4 className="text-xs text-slate-500 font-medium mb-1">Properties ({propertiesInThisGroup.length})</h4>
                        ...
                        {propertiesInThisGroup.length > 0 ? (
                          propertiesInThisGroup.map((property) => (
                            <Link
                              ...
                              key={property.id}
                              className="flex items-center p-2 rounded-md hover:bg-slate-50 transition-colors text-sm group/item"
                              onClick={(e) => e.stopPropagation()}
                            >
                              <Home className="h-4 w-4 text-slate-400 mr-2 shrink-0" />
                              <span className="flex-1 font-medium truncate" title={property.name || 'Unnamed Property'}>
                                {property.name || 'Unnamed Property'}
                              </span>
                              {/* Show property category icon */}
                              {property.property_category && (
                                <span className="text-xs text-slate-400 px-1.5 mr-1">
                              ...
                              <ChevronRight className="h-4 w-4 text-slate-300 ml-1 shrink-0 opacity-0 group-hover/item:opacity-100 transition-opacity" />
                            </Link>
                          ))
                        ) : (
                          <p className="text-sm text-slate-500 p-2">No properties currently in this group.</p>
                        )}
                      </div>
                    </div>
                    ...
                    {group.shared_items && group.shared_items.length > 0 && (
                      <div>
                        <h4 className="text-xs text-slate-500 font-medium mb-1">Shared Items ({group.shared_items.length})</h4>
                        <div className="mt-1 max-h-24 overflow-y-auto pr-1">
                          {group.shared_items.map((item) => (
                            <div key={item.id} className="flex items-center p-2 rounded-md text-sm">
                              <Box className="h-4 w-4 text-slate-400 mr-2 shrink-0" />
