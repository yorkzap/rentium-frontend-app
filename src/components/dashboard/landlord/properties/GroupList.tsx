      ...
      {groups.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-sm text-ink-3 mb-4">
              Create groups to manage rooms with shared areas.
            </p>
            <Button
              onClick={() =>
                router.push('/dashboard/properties/create?type=group')
              }
              variant="outline"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {groups.map((group) => {
            ...
            const propertiesInThisGroup =
              group.grouped_properties ||
              ...
              >
                  ...
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <DropdownMenu>
                      ...
                      >
                        ...
                        <DropdownMenuSeparator />
                        <DropdownMenuItem
                          onClick={(e) => onDeleteClick(e, group.id)}
                ...
                <CardContent className="py-3 flex-grow">
                  <div className="space-y-4">
                    {/* Properties Section */}
                    <div>
                      <h4 className="text-xs text-ink-3 font-medium mb-1">
                        Properties ({propertiesInThisGroup.length})
                      </h4>
                      <div className="mt-1 max-h-24 overflow-y-auto pr-1">
                        {propertiesInThisGroup.length > 0 ? (
                          ...
                          <p className="text-sm text-ink-3 p-2">
                            No properties currently in this group.
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    {group.shared_items && group.shared_items.length > 0 && (
                      <div>
                        <h4 className="text-xs text-ink-3 font-medium mb-1">
                          Shared Items ({group.shared_items.length})
                        </h4>
                        <div className="mt-1 max-h-24 overflow-y-auto pr-1">
                ...
                <CardFooter className="pt-0 pb-3 mt-auto">
                  <Link
                    href={`/dashboard/properties/view-group/${group.id}`}
                    className="w-full"
                    onClick={(e) => e.stopPropagation()}
                  >
