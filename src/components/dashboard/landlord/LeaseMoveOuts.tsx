...
import {
  DoorOpen, Loader2, FileSignature, CheckCircle2, XCircle, Plus, Info,
} from "lucide-react";
import { toast } from "sonner";
import { useAuth } from "@/contexts/AuthContext";
import {
  MoveOutRequest, MoveOutRules, RentHandling,
...
export default function LeaseMoveOuts({ leaseId, isActive }: { leaseId: string; isActive: boolean }) {
  const { token } = useAuth();
  ...
  const [initKind, setInitKind] = useState<"MUTUAL_AGREEMENT" | "LANDLORD_NOTICE">("MUTUAL_AGREEMENT");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRules(r); setRequests(reqs);
    } catch { /* non-active lease etc. */ }
  ...
  const act = async (fn: () => Promise<unknown>, okMsg: string) => {
    setBusy(true);
    try { await fn(); toast.success(okMsg); setShowDecline(false); setShowInitiate(false); load(); }
      ...
      <CardContent className="space-y-4">
        ...
        {pending && pending.initiated_by === "TENANT" && (
          <div className="rounded-md border border-amber-200 bg-amber-50 p-4 space-y-3 text-sm text-amber-900">
            <p className="font-medium flex items-center gap-1.5">
              ...
              {pending.tenant_name || "Tenant"} requests a {pending.form_type || "mutual agreement"} — your signature decides
            </p>
            <div className="rounded border bg-white p-3 space-y-1 text-slate-700">
              <p className="text-xs uppercase tracking-wide text-slate-400">Mutual Agreement to End a Tenancy {pending.form_type && `· ${pending.form_type}`}</p>
              <p>The tenant(s) agree to vacate the premises at <strong>1:00 PM</strong> on <strong>{fmt(pending.requested_end_date)}</strong>.</p>
              {pending.reason && <p className="text-slate-500">Tenant's reason: {pending.reason}</p>}
              <p className="text-xs text-slate-500 pt-1">
                Note: this is NOT a Notice to End Tenancy. Neither party is obliged to sign. By signing,
                both parties agree the tenancy ends on the date above with no further obligation between
                landlord and tenant. Signed by tenant {pending.tenant_signed_at ? new Date(pending.tenant_signed_at).toLocaleDateString() : ""}.
              </p>
            </div>
            <p className="text-xs">
              If you decline, nothing changes — the tenant owes rent through their full notice period
              (earliest standard end: {fmt(rules.earliest_tenant_end_date)}).
            </p>
            <div className="space-y-2">
              <Label className="text-xs">If you accept — the final month's rent:</Label>
              <Select value={rentHandling} onValueChange={(v) => setRentHandling(v as RentHandling)}>
                <SelectTrigger className="bg-white"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {RENT_OPTIONS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button size="sm" className="bg-slate-900 hover:bg-slate-800" disabled={busy}
                onClick={() => act(
                  () => acceptMoveOut(token!, pending.id, { rent_handling: rentHandling }),
            ...
            {showDecline && (
              <div className="space-y-2">
                <Textarea placeholder="Optional — reason shown to the tenant" value={declineReason}
                  onChange={(e) => setDeclineReason(e.target.value)} className="min-h-[60px] bg-white" />
                <Button size="sm" variant="destructive" disabled={busy}
                  onClick={() => act(() => declineMoveOut(token!, pending.id, declineReason),
        ...
        {pending && pending.initiated_by === "LANDLORD" && (
          <div className="rounded-md border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900 space-y-2">
            <p className="font-medium">Your {pending.kind_display.toLowerCase()} is awaiting the tenant's signature</p>
            <p>Proposed end: <strong>{fmt(pending.requested_end_date)}</strong> · Final month: {pending.rent_handling_display.toLowerCase()}.</p>
            <Button size="sm" variant="outline" disabled={busy}
              onClick={() => act(() => cancelMoveOut(token!, pending.id), "Proposal withdrawn.")}>
              Withdraw proposal
            </Button>
          </div>
        )}
        ...
        {!pending && isActive && (
          !showInitiate ? (
            <Button variant="outline" size="sm" onClick={() => setShowInitiate(true)}>
            ...
            <div className="rounded-md border p-4 space-y-3 text-sm">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Select value={initKind} onValueChange={(v) => setInitKind(v as typeof initKind)}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="MUTUAL_AGREEMENT">
                        Propose a {rules.mutual_agreement_form} mutual agreement (tenant must sign)
                      </SelectItem>
                      <SelectItem value="LANDLORD_NOTICE">Serve notice for landlord use (e.g. moving in)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input type="date" value={initDate} onChange={(e) => setInitDate(e.target.value)}
                    min={initKind === "LANDLORD_NOTICE" ? rules.earliest_landlord_end_date : rules.today} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input value={initReason} onChange={(e) => setInitReason(e.target.value)} placeholder="e.g. landlord moving in" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Final month's rent</Label>
                  <Select value={initRent} onValueChange={(v) => setInitRent(v as RentHandling)}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {RENT_OPTIONS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                    </SelectContent>
              ...
              <div className="flex gap-2">
                <Button size="sm" className="bg-slate-900 hover:bg-slate-800" disabled={busy || !initDate}
                  onClick={() => act(
                    () => createMoveOut(token!, {
                      lease: leaseId, kind: initKind, requested_end_date: initDate,
                      reason: initReason, rent_handling: initRent,
                    }),
                    initKind === "LANDLORD_NOTICE"
                  ...
                  {initKind === "LANDLORD_NOTICE" ? "Serve Notice" : "Send Proposal"}
                </Button>
              ...
              <li key={r.id} className="p-3 flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {r.status === "ACCEPTED"
                      ? <>Tenancy ends {fmt(r.effective_end_date)} · final month: {r.rent_handling_display.toLowerCase()}</>
                      : r.status === "DECLINED"
                      ? <>Requested {fmt(r.requested_end_date)}{r.decline_reason ? ` · "${r.decline_reason}"` : ""}</>
                      : <>Requested {fmt(r.requested_end_date)}</>}
                  </p>
                </div>
                <Badge variant="outline" className={statusBadge(r.status)}>{r.status_display}</Badge>
              </li>
            ))}
          </ul>
        )}
