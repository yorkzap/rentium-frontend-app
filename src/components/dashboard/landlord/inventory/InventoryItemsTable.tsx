...
export default function InventoryItemsTable({
  ...
  const getConditionBadgeVariant = (
    condition?: DisplayInventoryItem['condition']
  ...
  if (!items || items.length === 0) {
    return (
      <p className="text-center text-ink-3 py-6">No inventory items found.</p>
    );
  }
      ...
      <TableBody>
        {items.map((item) => (
          <TableRow key={`${item.itemType}-${item.id}`}>
            ...
            <TableCell>
              <Badge
                variant={item.itemType === 'shared' ? 'secondary' : 'outline'}
                className="text-xs"
              >
