...
import {
  ...
  FormLabel,
  FormMessage,
} from '@/components/ui/form'; // Removed FormDescription for checkbox
...
const itemSchemaBase = z.object({
  ...
  condition: z
    .enum(['NEW', 'GOOD', 'FAIR', 'POOR', 'DAMAGED', 'MISSING'])
    .nullable()
    .optional(),
...
type ItemApiPayload = Omit<
  z.infer<typeof itemSchemaBase>,
  'property' | 'group'
> & {
...
export default function InventoryItemForm({
  ...
  const form = useForm<z.infer<typeof itemSchemaBase>>({
    // Use base schema for form fields
  ...
  const handleFormSubmit = async (data: z.infer<typeof itemSchemaBase>) => {
    ...
    if (!targetId) {
      console.error(
        `Missing ${itemTypeToAddOrEdit === 'private' ? 'Property' : 'Group'} ID`
      );
      toast.error(
        `Cannot save item: Missing target ${itemTypeToAddOrEdit} ID.`
      );
      return;
    }
    ...
    const success = await onSubmit(
      payload,
      itemTypeToAddOrEdit,
      isEditMode ? itemToEdit?.id : undefined
    );
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          <DialogTitle>
            {isEditMode
              ? `Edit ${itemTypeToAddOrEdit}`
              : `Add New ${itemTypeToAddOrEdit}`}{' '}
            Item
          </DialogTitle>
          <DialogDescription>
            {isEditMode
              ? 'Update the details for this item.'
              : `Enter details for the new ${itemTypeToAddOrEdit} item.`}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="e.g., Kettle, Bedside Lamp"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder={
                        itemTypeToAddOrEdit === 'shared'
                          ? 'e.g., Shared Kitchen Counter'
                          : 'e.g., Bedroom 1 Closet'
                      }
                      {...field}
                      value={field.value ?? ''}
                    />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="e.g., Model #, purchase date, notes..."
                      {...field}
                      value={field.value ?? ''}
                    />
                  </FormControl>
