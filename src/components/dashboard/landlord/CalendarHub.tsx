...
import {
  Plus, ChevronLeft, ChevronRight, FileText, DollarSign, Wrench, Home, CalendarClock, Loader2,
} from "lucide-react"
import { useAuth } from "@/contexts/AuthContext"
import { useRouter } from "next/navigation"
import { toast } from "sonner"
import {
  fetchAgenda, createAgendaEvent, type AgendaItem, type AgendaType,
} from "@/lib/engagementApi"

const TYPE_STYLE: Record<AgendaType, { cls: string; icon: React.ReactNode; label: string }> = {
  lease_start: { cls: "bg-blue-100 text-blue-700", icon: <FileText className="h-3 w-3" />, label: "Lease start" },
  lease_end: { cls: "bg-purple-100 text-purple-700", icon: <FileText className="h-3 w-3" />, label: "Lease end" },
  charge_due: { cls: "bg-green-100 text-green-700", icon: <DollarSign className="h-3 w-3" />, label: "Payment due" },
...
export default function CalendarHub() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      setItems(await fetchAgenda(token, { start: iso(monthStart), end: iso(monthEnd) }))
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <AddEventDialog
        open={addOpen} token={token!} defaultMonth={cursor}
...
function AddEventDialog({ open, token, defaultMonth, onClose, onDone }: {
  open: boolean; token: string; defaultMonth: Date; onClose: () => void; onDone: () => void
}) {
  const [title, setTitle] = useState("")
  const [kind, setKind] = useState<"CUSTOM" | "INSPECTION" | "REMINDER" | "MOVE">("CUSTOM")
  const [startDate, setStartDate] = useState(iso(defaultMonth))
  const [notes, setNotes] = useState("")
  const [busy, setBusy] = useState(false)

  useEffect(() => {
    if (open) { setTitle(""); setKind("CUSTOM"); setStartDate(iso(new Date())); setNotes("") }
  }, [open])

  const submit = async () => {
    setBusy(true)
    try {
      await createAgendaEvent(token, { title, kind, start_date: startDate, notes })
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>A reminder, inspection, or note on your calendar.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label>Title</Label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Annual furnace inspection" />
          </div>
