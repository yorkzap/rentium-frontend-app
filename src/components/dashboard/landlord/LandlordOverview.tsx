...
import { Building2, Users, DollarSign, Wrench, FileText, ChevronRight, Loader2, AlertCircle, ImageIcon } from "lucide-react";
...
const getFullImageUrl = (relativeUrl: string | null | undefined): string | null => {
  ...
  if (djangoBaseUrl) {
    try {
      return relativeUrl.startsWith('/media/') ? `${djangoBaseUrl}${relativeUrl}` : new URL(relativeUrl, djangoBaseUrl).href;
    } catch { return null; }
  }
  return relativeUrl;
};
...
export default function LandlordOverview({ onNavigate }: { onNavigate: (section: string) => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const propsRes = await fetch(`${DJANGO_API_URL}/properties/`, { headers: { Authorization: `Token ${token}` } });
        if (!propsRes.ok) {
  ...
  const stats = [
    {
      ...
      value: isLoading && totalListings === 0 ? <Loader2 className="h-6 w-6 animate-spin" /> : totalListings.toString(),
  ...
  const overallOccupancyRate = totalUnits > 0 ? Math.round((occupiedCount / totalUnits) * 100) : 0;
  ...
  const taskItems = React.useMemo(() => {
    const tasks: { id: string; title: string; sub: string; type: "maintenance" | "lease"; navigate: string }[] = [];
    workOrders
      ...
      .forEach((w) =>
        tasks.push({
          id: `wo-${w.id}`,
          title: w.sla_breached ? `Overdue: ${w.title}` : `New request: ${w.title}`,
          sub: `${w.property_name}${w.is_rta_emergency ? " · RTA emergency" : ""}`,
          type: "maintenance", navigate: "maintenance",
        })
      );
  ...
  if (error) {
    return (
      <Card className="border-destructive bg-red-50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-destructive"><AlertCircle className="h-5 w-5" /> Error Loading Dashboard</CardTitle>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {stats.map((stat, index) => (
          <Card key={index} className="hover:shadow-md transition-all cursor-pointer border-l-4 border-transparent hover:border-l-blue-500" onClick={() => onNavigate(stat.navigate)}>
