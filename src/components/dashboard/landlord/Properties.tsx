...
export default function AssetManagement() {
  ...
  const getStatusColor = (status?: PropertyDetail['status']): string => {
    switch (status) {
      ...
      default:
        return 'bg-surface-sunken text-ink border-line';
    }
  };

  const getCategoryColor = (
    category?: PropertyDetail['property_category']
  ): string => {
    switch (category) {
      case 'COMPLETE_UNIT':
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const propPromise = fetch(`${DJANGO_API_URL}/properties/`, {
      ...
      // Process Properties
      if (!propResponse.ok)
        throw new Error(`Failed property fetch (${propResponse.status})`);
      ...
      // Process Groups
      if (!groupResponse.ok)
        throw new Error(`Failed group fetch (${groupResponse.status})`);
      ...
      setError(msg);
      toast.error(`Data Load Error: ${msg}`);
      setProperties([]);
      setGroups([]);
      setIsLoadingProps(false);
      setIsLoadingGroups(false);
  ...
  const getFilteredProperties = useCallback(() => {
    ...
    const searched = properties.filter(
      (p) =>
        p &&
    ...
    switch (categoryTab) {
      case 'units':
        return searched.filter((p) => p.property_category === 'COMPLETE_UNIT');
      case 'rooms':
        return searched.filter((p) => p.property_category === 'ROOM');
      case 'available':
        return searched.filter((p) => p.status === 'AVAILABLE');
      case 'occupied':
        return searched.filter((p) => p.status === 'OCCUPIED');
      case 'maintenance':
        return searched.filter((p) => p.status === 'MAINTENANCE');
      case 'not_available':
        return searched.filter((p) => p.status === 'NOT_AVAILABLE');
      default:
        return searched;
    }
  ...
  const categoryCounts = React.useMemo(
    () => ({
      ...
      units:
        properties?.filter((p) => p.property_category === 'COMPLETE_UNIT')
          .length ?? 0,
      rooms:
        properties?.filter((p) => p.property_category === 'ROOM').length ?? 0,
      available:
      ...
      not_available:
        properties?.filter((p) => p.status === 'NOT_AVAILABLE').length ?? 0,
    }),
    [properties]
  );
  ...
  const handleCreate = () =>
    router.push(
      `/dashboard/properties/create?type=${viewMode === 'groups' ? 'group' : 'property'}`
    );
  const handleEditProperty = (e: React.MouseEvent, id: number) => {
    e.stopPropagation();
  ...
  const handleDeleteGroupRequest = (id: string) => {
  ...
  const confirmDeleteProperty = async () => {
    ...
    try {
      ...
      toast.error(
        `Error deleting property: ${err instanceof Error ? err.message : 'Unknown'}`
      );
    } finally {
      setIsDeleting(false);
  ...
  const confirmDeleteGroup = async () => {
    ...
    const name =
      groups.find((g) => g.id === groupToDeleteId)?.name ?? 'Unknown Group';
    setIsDeleting(true);
    try {
      ...
      setProperties((prev) =>
        prev.map((p) =>
          p.group?.id === groupToDeleteId ? { ...p, group: null } : p
        )
      );
      setDeleteGroupDialogOpen(false);
    } catch (err) {
      toast.error(
        `Error deleting group: ${err instanceof Error ? err.message : 'Unknown'}`
      );
    } finally {
      setIsDeleting(false);
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-ink-2 text-sm mt-1">
            Overview and manage your property listings and groups.
          </p>
        </div>
        <div className="flex w-full sm:w-auto items-center space-x-2">
          <div className="relative flex-grow sm:flex-grow-0">
            ...
            <Input
              ...
              aria-label="Search properties or groups"
            />
          </div>
              ...
              >
                <TabsList className="inline-flex h-9 items-center justify-center rounded-lg bg-muted p-1 text-muted-foreground w-max">
                  ...
                  <TabsTrigger
                    ...
                    className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow"
                  >
                    Units ({categoryCounts.units})
                  </TabsTrigger>
                  <TabsTrigger
                    value="rooms"
                    className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow"
                  >
                    Rooms ({categoryCounts.rooms})
                  </TabsTrigger>
                  <TabsTrigger
                    value="available"
                    className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow"
                  ...
                  <TabsTrigger
                    ...
                    className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow"
                  >
                    Occupied ({categoryCounts.occupied})
                  </TabsTrigger>
                  <TabsTrigger
                    value="maintenance"
                    className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow"
                  ...
                  <TabsTrigger
                    ...
                    className="inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow"
                  >
                    Unavailable ({categoryCounts.not_available})
                  </TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
      ...
      <AlertDialog
        open={deletePropDialogOpen}
        onOpenChange={setDeletePropDialogOpen}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong className="mx-1 break-words">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel
              onClick={() => setDeletePropDialogOpen(false)}
              disabled={isDeleting}
            >
              Cancel
            </AlertDialogCancel>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              ? Properties within this group will become ungrouped. This cannot
              be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
