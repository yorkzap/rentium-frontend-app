// PublicPageSettings.tsx
//
// The "Public page" tab of Settings: the public-visibility switch, the exact
// disclosure contract, unpublishable-property warnings, and the page editor.
// Extracted from the old /dashboard/settings page so Settings could become one
// tabbed page (Account & RAMA + Public page).
...
import { Pill, Skeleton } from '@/components/ui/page';
  ...
  return (
    <div>
      ...
      <section className="card p-6">
        ...
        {(s.subdomain_url || s.public_url) &&
          (s.is_public ? (
            <Link
              href={(s.subdomain_url || s.public_url) as string}
              target="_blank"
            ...
            <p className="mt-4 text-sm text-[hsl(var(--ink-4))]">
              Your address once published:{' '}
              <span className="font-medium text-[hsl(var(--ink-2))]">
                {(s.subdomain_url || s.public_url)!.replace(/^https?:\/\//, '')}
              </span>
            </p>
          ))}
      </section>
        ...
        <div className="mt-4 space-y-4">
          <div>
            ...
            <div className="mt-1.5 flex items-center gap-0">
              ...
              />
              <span
                className="rounded-r-lg border border-l-0 bg-[hsl(var(--surface-sunken))] px-3 py-2 text-sm text-[hsl(var(--ink-4))]"
                style={{ borderColor: 'hsl(var(--line))' }}
              >
                .{process.env.NEXT_PUBLIC_ROOT_DOMAIN || 'rentium.ca'}
              </span>
            </div>
