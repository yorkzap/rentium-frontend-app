...
export default function InspectionSignCard({ leaseId }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const summaries = list.filter(
        (i) =>
          i.status === 'MOVE_IN_IN_PROGRESS' ||
          i.status === 'MOVE_OUT_IN_PROGRESS'
      );
      for (const summary of summaries) {
        ...
        const tenantSigned =
          pass === 'MOVE_IN'
            ? full.tenant_signed_move_in_at
            : full.tenant_signed_move_out_at;
        const landlordSigned =
          pass === 'MOVE_IN'
            ? full.landlord_signed_move_in_at
            : full.landlord_signed_move_out_at;
  ...
  const sections = useMemo(() => {
    if (!detail)
      return [] as { name: string; items: InspectionDetail['items'] }[];
  ...
  const handleSign = async () => {
    ...
    if (agrees === 'no' && !reason.trim()) {
      toast.error(
        "Please explain what you disagree with — it's recorded on the report."
      );
      return;
    }
    setIsSigning(true);
  ...
  return (
    <Card className="border-blue-200 bg-blue-50/40 mb-6">
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Your landlord recorded the condition of{' '}
          {pendingSummary.property_label || 'your unit'}. Review each item — if
          anything doesn&apos;t match what you see, you can disagree and say
          why; your disagreement is recorded on the report itself.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        >
          ...
          {expanded ? 'Hide' : 'Review'} the {detail.items.length} recorded
          items
        </Button>
                  ...
                  {detail.key_rows.map((k) => (
                    <li key={k.id} className="flex justify-between">
                      ...
                      <span className="text-slate-500">
                        {k.issued_count} issued
                        {k.returned_count !== null
                          ? ` · ${k.returned_count} returned`
                          : ''}
                      </span>
                    </li>
                  ))}
        ...
        <div className="space-y-3 rounded-md border bg-white p-4">
          ...
          <RadioGroup
            value={agrees}
            onValueChange={(v) => setAgrees(v as 'yes' | 'no')}
          >
          ...
          <div className="space-y-1.5">
            <Label htmlFor="insp-sign-name" className="text-sm">
            ...
            <p className="text-xs text-slate-500">
              Typing your name and clicking Sign records your electronic
              signature with a timestamp. Keep a copy of the report for your
              records.
            </p>
          </div>
          <Button
