...
import {
    AlertTriangle, Camera, CheckCircle2, ChevronRight, Clock, Loader2,
    MessageSquare, Plus, Send, Wrench, X,
} from "lucide-react";
...
import {
    addWorkOrderComment, addWorkOrderImage, createWorkOrder,
    fetchMaintenanceAreas, fetchWorkOrders,
    MaintenanceArea, WORK_ORDER_CATEGORIES, WORK_ORDER_PRIORITIES,
    WorkOrder, WorkOrderCategory, WorkOrderPriority,
...
export default function TenantMaintenance({ propertyId, leaseId, propertyLabel }: TenantMaintenanceProps) {
    const { token } = useAuth();
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const orders = await fetchWorkOrders(token, propertyId ? { property: propertyId } : {});
    ...
    const resetForm = () => {
        ...
        setPriority("MEDIUM"); setAreaId("none"); setPhotos([]);
    };

    const handleSubmit = async () => {
        if (!token || !propertyId) {
            toast.error("Missing property information — please refresh and try again.");
            return;
        }
        ...
        try {
            ...
            for (const file of photos) {
                try {
                    ...
                    toast.error(`Couldn't upload ${file.name} — you can add it from the request later.`);
                }
            }
    ...
    const handleAddComment = async () => {
        ...
        try {
            const comment = await addWorkOrderComment(token, selected.id, commentBody.trim());
    ...
    const openOrders = useMemo(() => workOrders.filter((w) => OPEN_STATUSES.includes(w.status)), [workOrders]);
    const closedOrders = useMemo(() => workOrders.filter((w) => !OPEN_STATUSES.includes(w.status)), [workOrders]);
    ...
    const row = (w: WorkOrder) => (
        ...
        >
            <div className={cn(
                ...
                w.priority === "EMERGENCY" ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-500",
            )}>
                {w.priority === "EMERGENCY" ? <AlertTriangle className="h-4 w-4" /> : <Wrench className="h-4 w-4" />}
            </div>
            <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                    <span className="truncate text-sm font-medium text-slate-900">{w.title}</span>
                    <Badge variant="outline" className={cn("hidden sm:inline-flex text-xs", PRIORITY_STYLES[w.priority])}>
                        {w.priority_display}
                    </Badge>
                </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <p className="text-xs text-slate-500">
                For urgent issues (no heat or water, major leak, electrical hazard, broken exterior lock),
                submit with <span className="font-medium">Emergency</span> priority <em>and</em> contact your
                landlord directly. If anyone is in danger, call emergency services first.
            </p>
            ...
            {isLoading ? (
                ...
                <Card>
                    <CardContent className="py-12 text-center">
                        ...
                        <h3 className="text-lg font-medium text-slate-700 mb-1">No maintenance requests yet</h3>
                        <p className="text-sm text-slate-500 mb-4">
                            Something broken, leaking, or not working{propertyLabel ? ` in ${propertyLabel}` : ""}? Let your landlord know.
                        </p>
                        <Button className="bg-slate-900 hover:bg-slate-800" onClick={openForm}>
                ...
                <div className="space-y-6">
                    <Card>
                        <CardHeader className="pb-3">
                            ...
                            <CardDescription>Your landlord sees these and updates their status as work progresses</CardDescription>
                        </CardHeader>
                        <CardContent className="p-0">
                            {openOrders.length > 0
                                ? openOrders.map(row)
                                : <p className="p-6 text-center text-sm text-slate-500">Nothing open right now.</p>}
                        </CardContent>
                    </Card>
            ...
            <Dialog open={isFormOpen} onOpenChange={(open) => { if (!isSubmitting) setIsFormOpen(open); }}>
                <DialogContent className="sm:max-w-[520px] max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Describe the issue{propertyLabel ? ` at ${propertyLabel}` : ""} — photos help a lot.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-2">
                        <div className="space-y-1.5">
                            <Label htmlFor="wo-title">What's the issue?*</Label>
                            <Input id="wo-title" placeholder="e.g. Kitchen sink is leaking" value={title}
                                   onChange={(e) => setTitle(e.target.value)} maxLength={200} />
                        </div>
                        <div className="space-y-1.5">
                            <Label htmlFor="wo-desc">Details*</Label>
                            <Textarea id="wo-desc" rows={4} value={description}
                                    ...
                                    <SelectContent>
                                        ...
                                        {areas.map((a) => (
                                            <SelectItem key={a.id} value={String(a.id)}>
                                                {a.name}{a.kind_display ? ` (${a.kind_display})` : ""}
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden"
                                   onChange={(e) => {
                                       ...
                                       if (files.length) setPhotos((prev) => [...prev, ...files].slice(0, 6));
                                       e.target.value = "";
                                   }} />
                            <div className="flex flex-wrap items-center gap-2">
                                {photos.map((f, i) => (
                                    <span key={`${f.name}-${i}`}
                                        ...
                                        <Camera className="h-3 w-3" /> {f.name.length > 22 ? `${f.name.slice(0, 20)}…` : f.name}
                    ...
                    <DialogFooter>
                        <Button type="button" variant="outline" disabled={isSubmitting} onClick={() => setIsFormOpen(false)}>
                            Cancel
                        </Button>
                        <Button type="button" className="bg-slate-900 hover:bg-slate-800" disabled={isSubmitting} onClick={handleSubmit}>
                        ...
                        <>
                            <DialogHeader>
                                ...
                                <DialogDescription className="flex flex-wrap items-center gap-2 pt-1">
                                    ...
                                    <Badge variant="outline" className={cn("text-xs", PRIORITY_STYLES[selected.priority])}>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-3 text-xs text-slate-500">
                                    <div>
                                        ...
                                        {format(parseISO(selected.created_at), "MMM d, yyyy")} by {selected.reported_by_name}
                                    </div>
                                    {selected.scheduled_date && (
                                        <div>
                                            <span className="block text-slate-400">Scheduled for</span>
                                            <span className="inline-flex items-center gap-1 text-slate-700 font-medium">
                                                <Clock className="h-3 w-3" />
                                                {format(parseISO(selected.scheduled_date), "MMM d, yyyy")}
                                            </span>
                                        </div>
                                    )}
                                        ...
                                        {selected.images.map((img) => (
                                            <a key={img.id} href={img.image} target="_blank" rel="noreferrer"
                                               className="block h-20 w-20 overflow-hidden rounded-md border">
                                                ...
                                                <img src={img.image} alt={img.caption || "Attachment"} className="h-full w-full object-cover" />
                                            </a>
                                        ))}
                                    </div>
                                )}
                                ...
                                <div className="border-t pt-3">
                                                ...
                                                <li key={c.id} className={cn(
                                                    ...
                                                    <div className="mb-0.5 flex items-center justify-between text-xs text-slate-500">
                                                        <span className="font-medium text-slate-700">
                                                            {c.author_name}{c.is_landlord ? " (Landlord)" : ""}
                                                        </span>
                                    ...
                                    {selected.status !== "CANCELLED" && (
                                        <div className="mt-3 flex items-start gap-2">
                                            <Textarea rows={2} value={commentBody} placeholder="Add a comment for your landlord..."
                                                      onChange={(e) => setCommentBody(e.target.value)} className="text-sm" />
                                            <Button size="icon" className="bg-slate-900 hover:bg-slate-800 flex-shrink-0"
                                                    disabled={isCommenting || !commentBody.trim()} onClick={handleAddComment}>
