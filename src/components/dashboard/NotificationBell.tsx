...
// polls the unread count every 60s (paused while the tab is hidden) and
// loads the feed when opened.
//
// Read semantics: OPENING the panel is reading. Everything visible is
// marked read server-side the moment the feed loads — no "mark all read"
// button to click, no per-item chore. The just-opened panel still renders
// the new items highlighted (so you can see what changed since last time);
// they render as read on the next open.
//
...
export default function NotificationBell() {
  ...
  // Opening IS reading: as soon as the feed is in front of the user we
  // clear the badge and mark everything read server-side. The items keep
  // their "new" highlight for THIS render only — is_read stays false in
  // local state so the user can still see what's new — and come back as
  // read the next time the panel opens.
    ...
    try {
      ...
      if (items.some((n) => !n.is_read)) {
        ...
        markAllNotificationsRead(token).catch(() => {
          /* the next unread-count poll will reconcile */
        });
      }
    } catch {
      setNotifications([]);
  ...
  const handleNotificationClick = (notification: AppNotification) => {
    // Already marked read on open — just deep-link.
    setIsOpen(false);
