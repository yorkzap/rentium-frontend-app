...
export function Sparkles({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 80 80" fill="none" className={cn("h-16 w-16", className)} aria-hidden>
      <path d="M14 22v12M8 28h12" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" />
      <path d="M62 10v8M58 14h8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" opacity="0.7" />
...
export function ScribbleRing({ className }: { className?: string }) {
  return (
    <svg
      ...
      fill="none"
      preserveAspectRatio="none"
      className={cn("absolute -inset-x-3 -inset-y-1 h-[calc(100%+8px)] w-[calc(100%+24px)] text-brand", className)}
      aria-hidden
    >
      <path
