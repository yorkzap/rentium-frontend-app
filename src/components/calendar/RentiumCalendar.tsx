...
export const KIND_STYLES: Record<CalEventKind, string> = {
  ...
  VIEWING_REQUEST: "bg-purple-50 text-purple-700 border-purple-300 border-dashed",
...
export function RentiumCalendar({
  events, onDayClick, selectedDay, rangeEnd, compact = false, footer,
  month: controlledMonth, onMonthChange,
}: {
  events: CalEvent[];
  ...
  const now = new Date();
  const [internal, setInternal] = useState({ year: now.getFullYear(), month: now.getMonth() });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-semibold text-slate-800">{monthLabel(year, month)}</h3>
        <div className="flex items-center gap-1">
          <Button variant="outline" size="icon" className="h-7 w-7" onClick={() => nav(-1)}>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                day && onDayClick && "hover:bg-teal-50 active:bg-teal-100 cursor-pointer",
                !day && "bg-slate-100/60 cursor-default",
                day && inRange(day) && "bg-teal-50",
                day === selectedDay && "ring-2 ring-inset ring-teal-500",
                day === rangeEnd && rangeEnd !== selectedDay && "ring-2 ring-inset ring-teal-300",
              )}
            >
              {day && (
                <>
                  <span className={cn(
                    ...
                    day === today ? "bg-teal-600 text-white font-semibold" : "text-slate-500",
                  )}>
                  ...
                  {dayEvents.length > 0 && (
                    <div className="mt-0.5 flex flex-wrap gap-0.5 sm:hidden">
                      {dayEvents.slice(0, 4).map((e) => (
                        <span key={e.id} className={cn("h-1.5 w-1.5 rounded-full", KIND_DOTS[e.kind])} />
                      ))}
                      {dayEvents.length > 4 && (
                    ...
                    {dayEvents.slice(0, maxChips).map((e) => (
                      <span key={e.id}
                        className={cn(
                          ...
                          KIND_STYLES[e.kind],
                          e.status === "PAID" && e.kind !== "PAYMENT" && "opacity-50 line-through",
                          e.status === "OVERDUE" && "border-red-300 bg-red-50 text-red-700",
                        )}>
                        {e.label}
                      </span>
                    ))}
                    {overflow > 0 && (
                      <span className="block text-[10px] text-slate-400 px-1">+{overflow} more</span>
                    )}
                  </div>
                </>
              )}
...
export function TenantCalendarCard({ leaseId }: { leaseId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      listAppointments(token, { lease: leaseId }).catch(() => [] as Appointment[]),
    ]).then(([entries, appts]) => {
      const evts: CalEvent[] = [];
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Rent and bill due dates, payments received, and scheduled visits.</CardDescription>
      </CardHeader>
      <CardContent>
        <RentiumCalendar
          events={events}
              ...
              <ul className="mt-3 space-y-1.5 text-sm">
                {dayEvents.map((e) => (
                  <li key={e.id} className="flex items-center justify-between gap-2">
                    <span className="text-slate-700 truncate">{e.detail || e.label}</span>
                    {e.status && <Badge variant="outline" className="text-xs flex-shrink-0">{e.status}</Badge>}
                  </li>
                ))}
              </ul>
            ) : selected ? (
              <p className="mt-3 text-sm text-slate-400">Nothing on {selected}.</p>
            ) : null
          }
        />
      </CardContent>
