...
**Province-awareness note:** `tenancy_rules` computes _requirements_; the
...
_State_ lives in Needs Attention — computed, persists until resolved.
_Events_ stay in the NotificationBell — fanout via `events/notify.py` ROUTES.
